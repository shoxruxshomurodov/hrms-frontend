...
class View extends Component {
  ...
  onDelete = (id) => {
    ...
    Swal.fire({
      title: t("Do you want to delete this business process ?"),
      showDenyButton: true,
          ...
          <div className="flex-display-more">
            <Breadcrumb
                titles={[
                  ...
                  { id: 3, title: t("View"), url: "/business-process" },
                  { id: 4, title: get(result, "title"), url: "" }
                ]}
            />
            <div>
              <Link
                  to={`/business-process/update/${get(result, "id")}`}
                  ...
                  <div className="col-md-6">
                    ...
                    <span className="d-block g-mr-10">
                      {t("Code")} : {get(result, "code")}
                        ...
                        <h5 className="d-flex justify-content-between align-items-center h5 mb-0">
                    <span className="d-block g-mr-10">
                      {t("Entity")} : {get(result, "entity")}
                    </span>
                        </h5>
                        <h5 className="d-flex justify-content-between align-items-center h5 mb-0">
                    <span className="d-block g-mr-10">
                      {t("Version tag")} : {get(result, "versionTag")}
                    </span>
                        </h5>
                      </div>
                    </div>
...
const mapStateToProps = (state) => {
  return {
    drawToRender: get(state, "normalizer.data.business-process-one.result", []),
    isFetched: get(state, "normalizer.data.business-process-one.isFetched", false),
...
const mapDispatchToProps = (dispatch) => {
  return {
    callToRender: (id) => {
      const storeName = "business-process-one";
    ...
    callToDelete: (attributes) => {
      dispatch({
        type: Actions.BUSINESS_PROCESS_DELETE.REQUEST,
        payload: {
          attributes,
          cb: {
            success: (nData, data) => {
