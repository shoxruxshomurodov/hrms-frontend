...
function DiplomaQualificationCreatePage(props) {
  const [isFetched, setIsFetched] = useState(false);
  const dispatch = useDispatch()
  const create = (attributes) => {
    setIsFetched("Create");
    dispatch({
      type: Actions.DIPLOMA_QUALIFICATION_CREATE.REQUEST,
      payload: {
        attributes,
        cb: {
          success: (nData, data) => {
...
export default withTranslation("HRMS")(DiplomaQualificationCreatePage);
