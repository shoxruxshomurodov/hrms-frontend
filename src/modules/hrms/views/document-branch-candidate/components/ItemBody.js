    ...
    return (
        ...
        {body.map((td, index) => (
            <tr
                ...
                onDoubleClick={() => history.push(`/DocumentBranchCandidate/view/${td.id}`)}
            >
                <td className={'text-left'}>{index + 1}</td>
                <td>{get(td, "title", "")}</td>
                <td>{t(get(td, "bpmnProcessNameTitle", ""))}</td>
                <td>{t(get(td, "vacancyRegionTitle", ""))}</td>
                <td>{t(get(td, "vacancyDistrictTitle", ""))}</td>
                <td>{t(get(td, "currentOnStructureTitle", ""))}</td>
                <td>{t(get(td, "vacancyPositionTitle", ""))}</td>
