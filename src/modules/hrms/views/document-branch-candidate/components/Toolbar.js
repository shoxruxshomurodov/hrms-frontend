...
import AsyncSelect from "../../../../../components/AyncSelect/AsyncSelect";
...
const ToolBarStructure = (props) => {
    ...
    return (
        <div className="media-md align-items-center g-mb-10 select-w-300 overflow-auto ">

            <div className="media d-md-flex align-items-center ml-auto">

                <AsyncSelect defaultValue={get(selected,'vacancyRegionId')} url={'region'}
                             ...
                             onChangeHandler={(data) => {
                                 onSelectFilter({
                                     selected: {...selected,vacancyRegionId:data},
                                     param: {vacancyRegionId: get(data, 'value', null)}
                                 })
                             }
                             }/>

                <AsyncSelect defaultValue={get(selected,'vacancyDistrictId')} url={`district?regionId=${get(selected,'vacancyRegionId.value')}`}

                             property={["id", "title", "type"]}
                             className={'g-ml-15 g-ml-55--md'}
                             placeholder={t('Select district')}
                             onChangeHandler={(data) => onSelectFilter({
                                 selected: {...selected,vacancyDistrictId:data},
                                 param: {vacancyDistrictId: get(data, 'value', null)}
                             })}/>

                <AsyncSelect defaultValue={get(selected,'currentOnStructureId')} url={'structure'}

                             property={["id", "title", "type"]}
                             className={'g-ml-15 g-ml-55--md'}
                             placeholder={t('Select structure')}
                             onChangeHandler={(data) => onSelectFilter({
                                 selected: {...selected,currentOnStructureId:data},
                                 param: {currentOnStructureId: get(data, 'value', null)}
                             })}/>
