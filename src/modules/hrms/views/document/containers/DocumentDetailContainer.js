...
const DocumentDetailContainer = ({
    ...
    const eSign = (res) => {
        ...
        Swal.fire({
            ...
            if (res.isConfirmed) {
                if (!isEmpty(pkcs7_64) && !isNil(name)) {
                    setLoading(true);
                    signDocumentWithEimzoRequest({
                        attributes: {
                            documentId: id,
    ...
    return (
        <>
            <Breadcrumb
                titles={[
                    {id: 1, title: t("Documents"), url: "/document-list"},
    ...
    return {
        getDocumentLink: (id) => {
            ...
            dispatch({
                ...
                payload: {
                    url: `document/get/all/documents/${id}`,
                    scheme,
                    storeName,
                    entityName
                }
...
export default withTranslation("HRMS")(connect(mapStateToProps, mapDispatchToProps)(DocumentDetailContainer));
