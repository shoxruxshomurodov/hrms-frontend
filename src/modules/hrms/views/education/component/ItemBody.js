    ...
    <tbody ref={ref}>
      {body && body.map((td, index) => (
        <tr
          ...
          onDoubleClick={() => history.push(`/education/view/${td.id}`)}
        >
          <td>{get(td, "id", "")}</td>
          <td>{get(td, "diplomaQualification.title", "")}</td>
          <td>{get(td, "educationalInstitution.title", "")}</td>
          <td>{get(td, "faculty.title", "")}</td>
          <td>{get(td, "formStudy.title", "")}</td>
          <td>{get(td, "speciality.title", "")}</td>
          <td>{get(td, "receiptDate", "")?.join("-")}</td>
          <td>
            {!isNil(get(td, "expirationDate", ""))
              ? get(td, "expirationDate", "")?.join("-")
              : "Tugallanmagan"}
          </td>
          <td>{get(td, "type", "")}</td>
        </tr>
