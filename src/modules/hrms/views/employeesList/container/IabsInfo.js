...
const IabsInfo = ({t,user,hasAction=false}) => {
    ...
    const attributes = [
        {label: t("Name"), value: get(iabs, 'lastName')+' '+get(iabs, 'firstName')+' '+get(iabs, 'middleName')},
        {label: t("Pinfl"), value: get(iabs, 'inps')},
        {label: t("passport"), value: get(iabs, 'passportSeria') + get(iabs, 'passportNumber')},
    ...
    return (
        <>
            <div>
                <div className='row'>
                    {!isNil(get(user,'altEmp',null)) ? <div className='col col-12'>
                        {!isFetched ? <><h2 className="h4 g-font-weight-300">Manage your Name, ID and Email Addresses</h2>
                                    ...
                                    return (
                                        ...
                                        <li key={index+1} className="d-flex align-items-center justify-content-between g-brd-bottom g-brd-gray-light-v4 g-py-15">
                                            <div className="g-pr-10">
                                                ...
                                                <span className="align-top">{get(item,'value')}</span>
                                            </div>
                                            <span><i
                        ...
                        {!hasAction && <div className={'text-sm-right'}>
                                <button onClick={refreshServices}
                                        className={"btn u-btn-darkgray rounded-0 g-py-12 g-px-25 g-mr-10"}>{t('Refresh')}</button>
                            </div>}
                        </>:<ContentLoader /> }
                    </div>:<div className={'col-12 p-5 text-center'}><h2 className="h4 g-font-weight-300">Not employee</h2></div>}
