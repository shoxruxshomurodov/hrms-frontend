...
class View extends Component {
  ...
  onDelete = (id) => {
    ...
    Swal.fire({
      title: t("Do you want to delete this faculty ?"),
      showDenyButton: true,
        ...
        <div className="flex-display-more">
          <Breadcrumb
            titles={[
              ...
              { id: 3, title: t("View"), url: "/faculty" },
              { id: 4, title: get(result, "title"), url: "" }
            ]}
          />
          <div>
            <Link
              to={`/faculty/update/${get(result, "id")}`}
...
const mapStateToProps = (state) => {
  return {
    drawToRender: get(state, "normalizer.data.faculty-one.result", []),
    isFetched: get(state, "normalizer.data.faculty-one.isFetched", false),
...
const mapDispatchToProps = (dispatch) => {
  return {
    callToRender: (id) => {
      const storeName = "faculty-one";
      const entityName = "faculty";
      const scheme = FacultyScheme;
      dispatch({
    ...
    callToDelete: (attributes) => {
      dispatch({
        type: Actions.FACULTY_DELETE.REQUEST,
        payload: {
          attributes,
          cb: {
            success: (nData, data) => {
