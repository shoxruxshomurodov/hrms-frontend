...
function FacultyUpdatePage(props) {
  const dispatch = useDispatch();
  const result = useSelector((state) =>
    get(state, "normalizer.data.faculty.result", [])
  );
  ...
  const update = (attributes) => {
    ...
    setIsFetched("Update");
    dispatch({
      type: Actions.FACULTY_UPDATE.REQUEST,
      payload: {
        id,
        attributes,
        cb: {
  ...
  const { t } = props;
  const { id } = props.match.params;
  const response = Normalizer.Denormalize(result, FacultyScheme, entities);
  const values = [
    {
      id: 1,
      label:"Title",
...
export default withTranslation("HRMS")(FacultyUpdatePage);
