...
class View extends Component {
  ...
  onDelete = (id) => {
    ...
    Swal.fire({
      title: t("Do you want to delete this gsp district ?"),
      showDenyButton: true,
        ...
        <div className="flex-display-more">
          <Breadcrumb
            titles={[
              ...
              { id: 3, title: t("View"), url: "/gsp-district" },
              { id: 4, title: get(result, "title"), url: "" }
            ]}
          />
          <div>
            <Link
              to={`/gsp-district/update/${get(result, "id")}`}
            ...
            <div className="col-md-6">
              <div className="media-body g-brd-around g-brd-gray-light-v4 g-pa-30">
                <div className="g-mb-15">
                  ...
                  <h5 className="d-flex justify-content-between align-items-center h5 mb-0">
                    <span className="d-block g-mr-10">
                      {t("District Id")} : {get(result, "district.id")}
                    </span>
                  </h5>
                  <h5 className="d-flex justify-content-between align-items-center h5 mb-0">
                    <span className="d-block g-mr-10">
                      {t("District Title")} : {get(result, "district.title")}
                    </span>
                  </h5>
                  <h5 className="d-flex justify-content-between align-items-center h5 mb-0">
                    <span className="d-block g-mr-10">
                      {t("GspCountry id")} : {get(result, "gspCountry.id")}
                    </span>
                  </h5>
                  <h5 className="d-flex justify-content-between align-items-center h5 mb-0">
                    <span className="d-block g-mr-10">
                      {t("GspCountry Title")} : {get(result, "gspCountry.title")}
                    </span>
                  </h5>

                  <h5 className="d-flex justify-content-between align-items-center h5 mb-0">
                    <span className="d-block g-mr-10">
                      {t("GspRegion id")} : {get(result, "gspRegion.id")}
                    </span>
                  </h5>
                  <h5 className="d-flex justify-content-between align-items-center h5 mb-0">
                    <span className="d-block g-mr-10">
                      {t("GspRegion Title")} : {get(result, "gspRegion.title")}
                    </span>
                  </h5>
                </div>
              </div>
...
const mapStateToProps = (state) => {
  return {
    drawToRender: get(state, "normalizer.data.gsp-district-one.result", []),
    isFetched: get(state, "normalizer.data.gsp-district-one.isFetched", false),
...
const mapDispatchToProps = (dispatch) => {
  return {
    callToRender: (id) => {
      const storeName = "gsp-district-one";
      const entityName = "gsp-district";
    ...
    callToDelete: (attributes) => {
      dispatch({
        type: Actions.GSP_DISTRICT_CONTROLLER_DELETE.REQUEST,
        payload: {
          attributes,
          cb: {
            success: (nData, data) => {
