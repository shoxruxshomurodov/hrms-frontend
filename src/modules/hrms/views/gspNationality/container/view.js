...
class View extends Component {
  ...
  onDelete = (id) => {
    ...
    Swal.fire({
      title: t("Do you want to delete this gsp nationality ?"),
      showDenyButton: true,
        ...
        <div className="flex-display-more">
          <Breadcrumb
            titles={[
              ...
              { id: 3, title: t("View"), url: "/gsp-nationality" },
              { id: 4, title: get(result, "title"), url: "" }
            ]}
          />
          <div>
            <Link
              to={`/gsp-nationality/update/${get(result, "id")}`}
            ...
            <div className="col-md-6">
              <div className="media-body g-brd-around g-brd-gray-light-v4 g-pa-30">
                <div className="g-mb-15">
                  ...
                  <h5 className="d-flex justify-content-between align-items-center h5 mb-0">
                    <span className="d-block g-mr-10">
                      {t("Nationality Id")} : {get(result, "nationality.id")}
                    </span>
                  </h5>
                  <h5 className="d-flex justify-content-between align-items-center h5 mb-0">
                    <span className="d-block g-mr-10">
                      {t("Nationality Title")} : {get(result, "nationality.title")}
                    </span>
                  </h5>
                </div>
              </div>
...
const mapStateToProps = (state) => {
  return {
    drawToRender: get(state, "normalizer.data.gsp-nationality-one.result", []),
    isFetched: get(state, "normalizer.data.gsp-nationality-one.isFetched", false),
...
const mapDispatchToProps = (dispatch) => {
  return {
    callToRender: (id) => {
      const storeName = "gsp-nationality-one";
      const entityName = "gsp-nationality";
    ...
    callToDelete: (attributes) => {
      dispatch({
        type: Actions.GSP_NATIONALITY_CONTROLLER_DELETE.REQUEST,
        payload: {
          attributes,
          cb: {
            success: (nData, data) => {
