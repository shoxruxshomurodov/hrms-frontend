...
class View extends Component {
  ...
  onDelete = (id) => {
    ...
    Swal.fire({
      title: t("Do you want to delete this nationality ?"),
      showDenyButton: true,
  ...
  onLink = (nationalityId) => {
    const {callToLink} = this.props;
    const {iabsCode} = this.state;
    callToLink({iabsCode,nationalityId})
  }
  render() {
        ...
        <div className="flex-display-more">
          <Breadcrumb
            titles={[
              ...
              { id: 3, title: t("View"), url: "/nationality" },
              { id: 4, title: get(result, "title"), url: "" }
            ]}
          />
          <div>
            <Link
              to={`/nationality/update/${get(result, "id")}`}
...
const mapStateToProps = (state) => {
  return {
    drawToRender: get(state, "normalizer.data.nationality-one.result", []),
    isFetched: get(state, "normalizer.data.nationality-one.isFetched", false),
...
const mapDispatchToProps = (dispatch) => {
  return {
    callToRender: (id) => {
      const storeName = "nationality-one";
    ...
    callToDelete: (attributes) => {
      dispatch({
        type: Actions.NATIONALITY_CONTROLLER_DELETE.REQUEST,
        payload: {
          attributes,
          cb: {
            success: (nData, data) => {
    ...
    callToLink: (attributes) => {
      dispatch({
        type: Actions.LINK_ABS_NATION_TO_CORE_NATIONALITY.REQUEST,
        payload: {
          attributes,
          cb: {
            success: (nData, data) => {
