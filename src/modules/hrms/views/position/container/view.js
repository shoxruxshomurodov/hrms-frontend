...
class View extends Component {
  ...
  onDelete = (id) => {
    ...
    Swal.fire({
      title: t("Do you want to delete position ?"),
      showDenyButton: true,
  ...
  onLink = (positionId) => {
    ...
    callToLink({iabsCode,positionId})
  }
  render() {
    const {iabsCode} = this.state
        ...
        <div className="flex-display-more">
          <Breadcrumb
            titles={[
              ...
              { id: 3, title: t("View"), url: "/position" },
              { id: 4, title: get(result, "title"), url: "" }
            ]}
          />
          <div>
            <Link
              to={`/position/update/${get(result, "id")}`}
                ...
                <div className="g-mb-15">
                  <div className="d-flex g-mb-15">
                    <SelectAsyncPagination
                        url="iabsHrSPosts/list"
                        attrSearch={"name"}
                        onChange={this.filterBy}
                        property={["postId","postName","postId"]}
                    />
                    <button
...
const mapStateToProps = (state) => {
  return {
    drawToRender: get(state, "normalizer.data.position-one.result", []),
    isFetched: get(state, "normalizer.data.position-one.isFetched", false),
...
const mapDispatchToProps = (dispatch) => {
  return {
    ...
    callToDelete: (attributes) => {
      dispatch({
        type: Actions.POSITION_CONTROLLER_DELETE.REQUEST,
        payload: {
          attributes,
          cb: {
            success: (nData, data) => {
    ...
    callToLink: (attributes) => {
      dispatch({
        type: Actions.LINK_ABS_POSITION_TO_CORE_POSITION.REQUEST,
        payload: {
          attributes,
          cb: {
            success: (nData, data) => {
