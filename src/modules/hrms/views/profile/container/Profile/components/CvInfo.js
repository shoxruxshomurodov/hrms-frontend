...
const CvInfo = ({
  ...
  return (
    <>
      {!isFetched ? (
        <div>
          {get(user, "employeeReference.resume.files", []).map(
            (pdf, index) =>
          ...
          <div className={"text-sm-right g-mt-30"}>
            ...
            {!isNil(
              find(get(user, "employeeReference.resume.files", []), (pdf) =>
...
const mapDispatchToProps = (dispatch) => {
  return {
    syncEmployeeCvInfo: ({ id, cb }) =>
      dispatch({
        type: Actions.ASYNC_EMPLOYEE_CV.REQUEST,
        payload: { id, cb },
      }),
  };
};
...
export default withTranslation("HRMS")(
  connect(mapStateToProps, mapDispatchToProps)(CvInfo)
);
