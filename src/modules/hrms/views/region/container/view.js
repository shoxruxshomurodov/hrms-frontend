...
class View extends Component {
  ...
  onDelete = (id) => {
    ...
    Swal.fire({
      title: t("Do you want to delete this region ?"),
      showDenyButton: true,
        ...
        <div className="flex-display-more">
          <Breadcrumb
            titles={[
              ...
              { id: 3, title: t("View"), url: "/region" },
              { id: 4, title: get(result, "title"), url: "" }
            ]}
          />
          <div>
            <Link
              to={`/region/update/${get(result, "id")}`}
...
const mapStateToProps = (state) => {
  return {
    drawToRender: get(state, "normalizer.data.region-one.result", []),
    isFetched: get(state, "normalizer.data.region-one.isFetched", false),
...
const mapDispatchToProps = (dispatch) => {
  return {
    ...
    callToDelete: (attributes) => {
      dispatch({
        type: Actions.REGION_CONTROLLER_DELETE.REQUEST,
        payload: {
          attributes,
          cb: {
            success: (nData, data) => {
    ...
    callToLink: (attributes) => {
      dispatch({
        type: Actions.LINK_ABS_REGION_TO_CORE_REGION.REQUEST,
        payload: {
          attributes,
          cb: {
            success: (nData, data) => {
