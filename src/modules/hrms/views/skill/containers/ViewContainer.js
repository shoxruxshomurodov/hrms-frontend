...
const ViewContainer = ({
    ...
    const onDelete = (id) => {
        Swal.fire({
            title: t("Do you want to delete this skill?"),
            showDenyButton: true,
                ...
                Api.skillDelete(id)
                    .then((_res) => {
                        Swal.fire({
                            position: "center",
                            icon: "success",
                            title: t("Your skill has been deleted"),
    ...
    return (
        <>
            <div className="flex-display-more">
                ...
                <div>
                    <Link
                        to={`/skill/update/${get(result, "id", null)}`}
...
const mapStateToProps = (state) => {
    return {
        drawToRender: get(state, "normalizer.data.skill-one.result", null),
        isFetched: get(state, "normalizer.data.skill-one.isFetched", false),
