...
class View extends Component {
  ...
  onDelete = (id) => {
    ...
    Swal.fire({
      title: t("Do you want to delete this speciality ?"),
      showDenyButton: true,
        ...
        <div className="flex-display-more">
          <Breadcrumb
            titles={[
              ...
              { id: 3, title: t("View"), url: "/speciality" },
              { id: 4, title: get(result, "title"), url: "" }
            ]}
          />
          <div>
            <Link
              to={`/speciality/update/${get(result, "id")}`}
...
const mapStateToProps = (state) => {
  return {
    drawToRender: get(state, "normalizer.data.speciality-one.result", []),
    isFetched: get(state, "normalizer.data.speciality-one.isFetched", false),
...
const mapDispatchToProps = (dispatch) => {
  return {
    callToRender: (id) => {
      const storeName = "speciality-one";
    ...
    callToDelete: (attributes) => {
      dispatch({
        type: Actions.SPECIALITY_DELETE.REQUEST,
        payload: {
          attributes,
          cb: {
            success: (nData, data) => {
